...
const MyDesktopView: React.FC<MyDesktopViewProps> = ({ installedApps, setPage }) => {
        ...
        <div className="grid grid-cols-4 gap-2 flex-1">
          {['7', '8', '9', '/', '4', '5', '6', '*', '1', '2', '3', '-', 'C', '0', '=', '+'].map(btn => (
            <button
              ...
              className={`rounded-lg font-bold text-xl transition-colors active:scale-95 ${btn === 'C' ? 'bg-red-500 hover:bg-red-600' :
  ...
  const desktopApps = useMemo(() => [
    ...
    { id: 'marketplace', title: 'Marketplace', icon: Grid, content: <MarketplaceView setPage={setPage} toggleInstall={() => { }} />, width: 900, height: 600 },
          ...
          <div className="flex items-center gap-1 overflow-x-auto hide-scrollbar">
            {windows.map(win => (
              <button
                ...
                className={`flex items-center gap-2 px-3 py-1.5 rounded-md transition-all max-w-[200px] flex-shrink-0 group ${win.id === activeWindowId
